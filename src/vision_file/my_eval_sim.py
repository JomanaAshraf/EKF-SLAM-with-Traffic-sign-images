...
def my_evaluate(device,model,int_to_label,normalize,resize,to_tensor,original_image, min_score=0.45, max_overlap=0.3, top_k=200, annotate_image=True):
    ...
    det_boxes_center=xy_to_cxcy(det_boxes)
    ...
    if det_labels != ['background']:
        for i in range(det_boxes.size(0)):
            ...
            box_location_center= det_boxes_center[i].tolist()
            dic[det_labels[i]]=box_location_center
            if annotate_image:
                ...
                # annotated_image.save('./' +'perception'+ '.png')
